...
from modules.inception_tool_helpers import ReadOnceRegistry, ToolCallLoopDetector, recover_tool_args, tool_result_failed
...
def _chat_postcheck_enabled() -> bool:
    return os.getenv("OPRAI_CHAT_POSTCHECK_ENABLED", "0").strip().lower() in (
        "1",
        "true",
        "yes",
    )
...
def _chat_postcheck_retry_limit() -> int:
    raw = os.getenv("OPRAI_CHAT_POSTCHECK_RETRY_LIMIT", "1").strip()
    try:
...
def _needs_chat_postcheck(content: str) -> bool:
    text = (content or "").strip()
    if not text:
        return True
    lowered = text.lower()
    # Conservative guard: only one corrective retry for obvious empty/error-like outputs.
    bad_markers = (
        "llmrouter error:",
        "error:",
        "failed",
...
def call_chat(
    ...
    try:
        ...
        if not content and message.get("tool_calls"):
            content = ""
        retries_left = _chat_postcheck_retry_limit() if _chat_postcheck_enabled() else 0
        while retries_left > 0 and _needs_chat_postcheck(content):
            retries_left -= 1
            postcheck_thread = list(thread)
            postcheck_thread.append(
                {
                    "role": "assistant",
            ...
            postcheck_thread.append(
                {
                    "role": "user",
                    "content": (
                        "Post-check: answer must be factual, concise, and free of tool/provider error wrappers. "
                        "If evidence is missing, explicitly say so."
                    ),
                }
            )
            retry_payload = dict(payload)
            retry_payload["messages"] = postcheck_thread
            retry_status, retry_data, retry_err = _post_chat(retry_payload, timeout_seconds)
            if retry_status != 200:
                break
            retry_msg = retry_data.get("choices", [{}])[0].get("message", {}) or {}
...
def call_agent(
    ...
    inject_evidence_bundle_block(thread, runtime, user_text)
    ...
    strict_read_once = bool(
        deliverable_path and (roadmap_task or audit_task) and not runtime.consult_only
    )
    read_once = ReadOnceRegistry() if strict_read_once else None
    _default_high_tolerance = (
        "read_file",
        "grep_search",
        "list_directory",
        "glob_search",
    ...
    loop_detector = ToolCallLoopDetector(
        identical_threshold=2 if strict_read_once else 3,
        high_tolerance_tools=() if strict_read_once else _default_high_tolerance,
    )
    started = time.monotonic()
    steps = 0
    ...
    try:
        for _ in range(max_steps):
            ...
            write_only = should_write_only_tools(
                runtime,
                user_text,
            ...
            if synthesis_step:
                ...
                step_tools = all_tool_schemas
                if write_only:
                    step_tools = [
                        t
                        for t in all_tool_schemas
                        if (t.get("function") or {}).get("name") == "write_file"
                    ]
                payload["tools"] = step_tools
                if write_only and step_tools:
                    payload["tool_choice"] = "required"
                    forced_tool_used = True
                elif forced_first and steps == 0 and _force_audit_tool_enabled():
                    payload["tool_choice"] = "required"
                    forced_tool_used = True
                else:
                    payload["tool_choice"] = "auto"
            ...
            for tool_call in tool_calls:
                ...
                if args.get("_parse_error"):
                    ...
                    blocked = read_once.check_read(name, args)
                    if blocked:
                        result = json.dumps(
                            {"error": blocked, "next_required_tool": "write_file"}
                        )
                    else:
                        result = execute_tool(name, args, runtime)
                        if not tool_result_failed(result):
                            read_once.record_read(name, args)
                else:
                    result = execute_tool(name, args, runtime)
                ...
                if activity_enabled() and request_id and _log:
                    ...
                    try:
                        result_hash = hashlib.sha256(str(result).encode("utf-8", errors="replace")).hexdigest()[:12]
                    except Exception:
                        result_hash = "hash_error"
                    _log(
                        request_id,
                        "guard",
                        json.dumps(
                            {
                                "event": "adapter_tool_after_execute",
                                "request_id": request_id,
                                "tool": name,
